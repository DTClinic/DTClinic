...
def test_single_replica_parallel():
	...
	model = DTClinic.DistributedDataParallel(model)
...
def test_data_parallel():
	...
	#sampler = DTClinic.DistributedSampler(dataset, shuffle=True)
	dataloader = DTClinic.DataLoader(
	    dataset, batch_size=32, num_workers=1, drop_last=False)
	model = torch.nn.Linear(1, 1, bias=True)
	#model = model.to(device)
	params = [model.bias, model.weight]
	sgd = torch.optim.SGD(
	    ...
	    lr=0.01)
	schedule = torch.optim.lr_scheduler.MultiStepLR(sgd, [50])
	model = DTClinic.DataParallel(model)
	#model = DTClinic.DataParallel(model, device_ids=[0, 1])
...
test_data_parallel()
DTClinic.debug_test(test_data_parallel)
#DTClinic.debug_test(test_single_replica_parallel)
